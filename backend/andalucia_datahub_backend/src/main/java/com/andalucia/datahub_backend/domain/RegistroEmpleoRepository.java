...
public interface RegistroEmpleoRepository {
    // 1. Guardar un registro nuevo de empleo
    RegistroEmpleos guardar(RegistroEmpleos registroEmpleos);

    // 2. Obtener todos los registros de empleo por filtros
    // 2.1 Buscar por año
    List<RegistroEmpleos> buscarPorAnio(Integer anio);
    // 2.2 Buscar por año y trimestre
    List<RegistroEmpleos> buscarPorAnioAndTrimestre(Integer anio,Integer trimestre);
